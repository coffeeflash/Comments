...
async function setUp(){
    ...
    let response = await fetch(baseUrl + '/api/comments?source=' + document.URL)
...
function fadeOut(obj, callback){
    ...
    const timer = setInterval(function(){
        if(opacity < 0.1){
            clearInterval(timer);
            if(typeof (callback) !== 'undefined') callback() //this executes the callback function!
        }
...
async function sendComment(){
    ...
    let quiz = await quizResponse.json()
    ...
    let response = await fetch(baseUrl + '/api/comment', {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({
                    sourceTitle: document.getElementById('comment-title').innerText,
                    source: document.URL,
